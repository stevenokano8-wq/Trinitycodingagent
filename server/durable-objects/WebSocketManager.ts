 ...
 *
 * Features:
 *   • Session-tagged broadcast (send to all clients in one session)
 *   • Global broadcast (send to all connected clients)
 *   • Topic subscriptions (clients opt-in to specific channels)
 *   • Connection metadata (userId, sessionId, topics)
 *   • REST API for server-side broadcast from other DOs
 *
 * Routes:
 *   GET  /ws?sessionId=…&userId=… — WebSocket upgrade
 *   POST /broadcast               — { sessionId?, topic?, message } server→clients
...
export class WebSocketManager extends Agent<AppEnv> {
  ...
  async onConnect(connection: Connection<ClientMeta>): Promise<void> {
    // Parse query params from the connection URL stored in connection context
  ...
  async onClose(connection: Connection<ClientMeta>): Promise<void> {
    // Optionally notify others in the same session
    const meta = connection.state as ClientMeta | undefined;
    if (!meta) return;
    for (const c of this.getConnections<ClientMeta>()) {
