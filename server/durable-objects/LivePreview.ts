 ...
 *
 * Routes:
 *   POST /prepare?sessionId=…    — detect framework + build manifest
 *   GET  /status?sessionId=…     — current preview status
 *   GET  /manifest?sessionId=…   — full asset manifest
 *   POST /deploy?sessionId=…     — push assets to R2 + return preview URL
 */
...
export class LivePreview {
  ...
  private async prepare(sessionId: string): Promise<Response> {
    ...
    // Read package.json from SESSION_WORKSPACE
  ...
  private async deploy(sessionId: string): Promise<Response> {
    ...
    if (!this.env.FILES_R2) {
      // R2 not yet connected; return a placeholder preview
    ...
    // Pull built files from workspace and push to R2
    if (this.env.SESSION_WORKSPACE) {
      const wsId  = this.env.SESSION_WORKSPACE.idFromName(sessionId);
      const stub  = this.env.SESSION_WORKSPACE.get(wsId);
