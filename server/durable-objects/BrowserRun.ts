 ...
 * Falls back to a structured response when BROWSER binding is unavailable
 * so the worker still deploys in environments that haven't enabled the binding.
 *
 * Routes:
 *   POST /navigate          — { url } → { title, status, content }
 *   POST /screenshot        — { url, fullPage? } → { dataUrl, width, height }
 *   POST /evaluate          — { url, script } → { result }
 *   POST /test              — { url, assertions[] } → { passed, results[] }
 *   POST /fill-and-submit   — { url, fields{}, submitSelector } → { success }
 *   GET  /sessions          — list browser sessions (last 20)
 */
