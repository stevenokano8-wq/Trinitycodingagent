 ...
 * and selects either "gemini-3.6-flash" (Flash) or "gemini-3.1-pro-preview" (Pro).
 */
export function routeLLMTask(
  ...
  let finalReason = "";
  if (reasons.length === 0) {
    finalReason = `Standard responsive execution routed to 'Llama 3.3' for lightning-fast delivery.`;
  } else {
    const briefReasons = reasons.join("; ");
    finalReason = complexity === "complex"
      ? `Routed to 'DeepSeek R1' due to high-complexity components: ${briefReasons}`
      : `Routed to 'Llama 3.3' for simple/visual task handling: ${briefReasons}`;
  }
