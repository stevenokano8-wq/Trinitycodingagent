...
const EnvBoxView = lazy(() => import("./components/EnvBoxView.tsx"));
...
function AppShell({
  ...
  const renderLeftPane = () => {
    ...
    switch (activeTab) {
      ...
      case "env": return wrapMotion("env", <EnvBoxView />);
...
function AppShellComponent() {
  ...
  const handleClearAllHistory = () => {
    ...
    sessionStorage.removeItem("trinity_tab_session_id");
    setSavedSessions([]);
...
export default function App() {
  ...
  const connectSSE = () => {
    ...
    es.addEventListener("task-update", (e: any) => {
      const t = JSON.parse(e.data) as Task;
      setThinkingState(null);
      setTasks(prev => { const idx = prev.findIndex(p => p.id === t.id); if (idx >= 0) { const next = [...prev]; next[idx] = t; return next; } return [...prev, t]; });
    });
