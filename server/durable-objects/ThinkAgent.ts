 ...
 *
 * Built-in tools (all Cloudflare-native — no external APIs):
 *   read(path)                — read file from SESSION_WORKSPACE
 *   write(path, content)      — write / create file
 *   edit(path, old, new)      — precise in-place string replacement
 *   delete(path)              — remove file
 *   list(path?)               — list files (optionally under prefix)
 *   find(pattern)             — fuzzy-find files by name
 *   grep(pattern, path?)      — search content across files
 *   bash(command)             — execute command via @cloudflare/sandbox
 *   ask_ai(prompt, taskType)  — call AI_GATEWAY for reasoning / code
 *   done(answer)              — terminate loop with final answer
 *
 * WebSocket protocol:
 *   → { type: "run", goal, sessionId }      start a task
 *   ← { type: "thought",   text }           agent reasoning
 *   ← { type: "tool_call", tool, args }     before tool execution
 *   ← { type: "tool_result", tool, result } after tool execution
 *   ← { type: "done",      answer }         final answer
 *   ← { type: "error",     message }        unrecoverable error
 *
 * REST routes:
 *   POST /run                 — start task, returns { answer } synchronously
...
export class ThinkAgent extends Agent<AppEnv, AgentState> {
  ...
  async onConnect(connection: Connection<unknown>): Promise<void> {
    // -ignore — Agent.ctx.storage accessed at runtime
    // @ts-ignore — Cloudflare runtime types resolved at deploy time
  ...
  async onMessage(connection: Connection<unknown>, raw: string): Promise<void> {
    ...
    if (data.type === "run") {
      ...
      // Run the agent loop, streaming events back over WS
  ...
  // ── AI call (CF Workers AI via AI_GATEWAY if available, else direct) ───────
  private async callAI(messages: AiChatMessage[], sessionId: string): Promise<string> {
    // Prefer AI_GATEWAY for caching + rate limiting
    // @ts-ignore — Cloudflare runtime types resolved at deploy time
    if (this.env.AI_GATEWAY) {
      const gwId  = // -ignore — env accessible via Agent base class at runtime
 // @ts-ignore — Cloudflare runtime types resolved at deploy time
 this.env.AI_GATEWAY.idFromName(`gw:${sessionId}`);
      const stub  = // -ignore — env accessible via Agent base class at runtime
 // @ts-ignore — Cloudflare runtime types resolved at deploy time
 this.env.AI_GATEWAY.get(gwId);
  ...
  private async executeTool(
    ...
    switch (tool) {
      ...
      case "bash": {
        ...
        // Use @cloudflare/sandbox when SANDBOX container binding is available
        // @ts-ignore — Cloudflare runtime types resolved at deploy time
        if (this.env.SANDBOX) {
          try {
            // -ignore — getSandbox signature varies by version
            // @ts-ignore — Cloudflare runtime types resolved at deploy time
        ...
        // Fallback: safe simulation for read-only commands
  ...
  // ── Safe bash simulation (no SANDBOX binding) ─────────────────────────────
  private async simulateBash(command: string, sessionId: string): Promise<string> {
    const cmd = command.trim();

    // ls / find equivalents — proxy to workspace
