 ...
 *
 * Exposes API routes and re-exports all Durable Objects from src/durable/.
 */
...
import { AppEnv, QueueMessage, MessageBatch, setRuntimeOverrides, resolveEnvWithOverrides } from "../server/env.js";
import { DatabaseStatus, Message, FileNode, Task } from "./types.js";

// ── Durable Object exports ────────────────────────────────────────────────────
export { SessionWorkspace }      from "./durable/SessionWorkspace.js";
export { FileExplorer }          from "./durable/FileExplorer.js";
export { WebSocketManager }      from "./durable/WebSocketManager.js";
export { WorkflowEngineSql as WorkflowEngine }        from "./durable/WorkflowEngine.js";
export { WorkflowEngineSql }                         from "./durable/WorkflowEngine.js";
export { ThinkAgent }            from "./durable/ThinkAgent.js";
export { SubAgentOrchestratorSql as SubAgentOrchestrator } from "./durable/SubAgentOrchestrator.js";
export { SubAgentOrchestratorSql }                         from "./durable/SubAgentOrchestrator.js";
export { UserProfile }           from "./durable/UserProfile.js";
export { WorkspaceRegistry }     from "./durable/WorkspaceRegistry.js";
export { AiGateway }             from "./durable/AiGateway.js";
...
function proxyToDO(
  ns: import("../server/env.js").DurableObjectNamespace,
  id: string,
  path: string,
  request: Request,
): Promise<Response> {
...
app.get("/api/session/:id/files", async (c) => {
  ...
  const wsId = c.env.SESSION_WORKSPACE.idFromName(sessionId);
  const stub = c.env.SESSION_WORKSPACE.get(wsId);
...
app.post("/api/session/:id/files", async (c) => {
  ...
  const wsId = c.env.SESSION_WORKSPACE.idFromName(sessionId);
  const stub = c.env.SESSION_WORKSPACE.get(wsId);
  const res = await stub.fetch(new Request("https://do/file", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
app.get("/api/session/:id/preview", async (c) => {
  if (!c.env.LIVE_PREVIEW) return c.json({ error: "not bound" }, 503);
  const sessionId = c.req.param("id");
  const lpId = c.env.LIVE_PREVIEW.idFromName(sessionId);
  const stub = c.env.LIVE_PREVIEW.get(lpId);
  const res = await stub.fetch(new Request(`https://do/status?sessionId=${sessionId}`));
  return new Response(res.body, { status: res.status, headers: res.headers });
});
...
app.get("/api/session/:id/ws", async (c) => {
  ...
  const wmId = c.env.WEBSOCKET_MANAGER.idFromName("global");
  const stub = c.env.WEBSOCKET_MANAGER.get(wmId);
  return stub.fetch(new Request(`https://do/?sessionId=${sessionId}`, req));
});

app.post("/api/session/:id/deploy", async (c) => {
  if (!c.env.LIVE_PREVIEW) return c.json({ error: "not bound" }, 503);
  const sessionId = c.req.param("id");
  const lpId = c.env.LIVE_PREVIEW.idFromName(sessionId);
  const stub = c.env.LIVE_PREVIEW.get(lpId);
  const res = await stub.fetch(new Request(`https://do/deploy?sessionId=${sessionId}`, { method: "POST" }));
  return new Response(res.body, { status: res.status, headers: res.headers });
});
