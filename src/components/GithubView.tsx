...
export default function GithubView({ sessionId }: GithubViewProps) {
  ...
  // Level 3 PR Autonomy State
  const [isCreatingPR, setIsCreatingPR] = useState(false);
  const [prResult, setPrResult] = useState<{ prUrl?: string; prNumber?: number; branch?: string } | null>(() => {
    const saved = localStorage.getItem(`trinity_pr_${activeSessionId}`);
    return saved ? JSON.parse(saved) : null;
  });

  // Whenever prResult changes, save to localStorage
  useEffect(() => {
    if (prResult) {
      localStorage.setItem(`trinity_pr_${activeSessionId}`, JSON.stringify(prResult));
    } else {
      localStorage.removeItem(`trinity_pr_${activeSessionId}`);
    }
  }, [prResult, activeSessionId]);
  ...
  const handleCreatePullRequest = async () => {
    if (!repoUrl) {
      setPushLogs(["[ERROR] Target Git Repository URL is required to create a Pull Request."]);
      setPushStatus("error");
      return;
    }
    ...
    setPushStatus("idle");
    setPushLogs(["[SYSTEM] Initiating Level 3 Autonomous Pull Request flow...", "[SYSTEM] Discovering base commits...", "[SYSTEM] Staging file diffs..."]);
    ...
    try {
      ...
      if (data.success) {
        ...
        // Append a PR commit record
        const randomSha = Math.random().toString(16).substring(2, 9);
        const newCommits = [
          {
            sha: randomSha,
            message: `🤖 Opened autonomous Pull Request #${data.prNumber} on branch ${data.branch}`,
      ...
      setPushStatus("error");
      setPushLogs(prev => [...prev, `[CRITICAL ERROR] PR creation failed: ${err.message}`]);
    } finally {
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                {/* Level 3 Autonomy: Pull Request Center */}
                <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-xs">
                  <div className="flex items-center justify-between mb-5">
                    <div className="flex items-center gap-2">
                      <GitBranch className="h-4 w-4 text-amber-500 animate-pulse" />
                      <h3 className="text-xs font-bold text-zinc-800 uppercase tracking-wider font-mono">Level 3 Autonomy: PR Center</h3>
                    </div>
                    <span className="text-[9px] bg-amber-50 border border-amber-200 text-amber-600 font-bold px-2 py-0.5 rounded-full font-mono">STANDARDS 2026</span>
                  </div>

                  <p className="text-xs text-zinc-500 leading-relaxed mb-4">
                    Verify the agent's ability to autonomously stage changes, compile patches, create branches, and open a complete GitHub Pull Request.
                  </p>

                  <div className="space-y-4">
                    {prResult ? (
                      <div className="p-4 bg-emerald-50/50 border border-emerald-250 rounded-2xl space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] uppercase font-bold text-emerald-600 font-mono flex items-center gap-1">
                            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-ping" />
                            PULL REQUEST OPENED
                          </span>
                          <span className="text-[10px] text-zinc-400 font-mono">PR #{prResult.prNumber}</span>
                        </div>
                        
                        <div className="space-y-1">
                          <p className="text-xs font-bold text-zinc-800 truncate">Branch: <span className="font-mono text-[11px] bg-zinc-100 text-zinc-600 px-1 rounded">{prResult.branch}</span></p>
                          <a 
                            href={prResult.prUrl} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="text-xs text-amber-500 hover:text-amber-600 font-semibold underline flex items-center gap-1 inline-flex mt-1"
                          >
                            View Pull Request on GitHub <ArrowUpRight className="h-3.5 w-3.5" />
                          </a>
                        </div>

                        <button
                          onClick={() => setPrResult(null)}
                          className="w-full bg-white hover:bg-zinc-50 border border-zinc-250 text-zinc-650 font-bold py-2 rounded-xl text-[10.5px] transition-colors"
                        >
                          Clear Saved PR State
                        </button>
                      </div>
                    ) : (
                      <div className="p-4 bg-zinc-50 border border-zinc-150 rounded-2xl text-center space-y-3">
                        <p className="text-[11px] text-zinc-400 font-mono">No active Pull Request opened for this session.</p>
                        
                        <button
                          onClick={handleCreatePullRequest}
                          disabled={isCreatingPR || isPushing}
                          className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-2 transition-all cursor-pointer shadow-sm active:scale-98 animate-pulse"
                        >
                          {isCreatingPR ? (
                            <>
                            ...
                            <>
                              <GitBranch className="h-3.5 w-3.5 fill-white" />
                              Autonomously Open Pull Request
                            </>
                          )}
                        </button>
                      </div>
