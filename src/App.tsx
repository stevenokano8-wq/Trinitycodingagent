...
export default function App() {
  ...
  useEffect(() => {
    ...
    eventSource.addEventListener("agent_suspended", (e: any) => {
      try {
        const data = JSON.parse(e.data);
        setSuspendedFrame(data.frame);
      } catch (_) {}
    });

    eventSource.addEventListener("agent_resumed", () => {
      setSuspendedFrame(null);
    });
  };
        ...
        <div className="bg-amber-50 border-b border-amber-200 px-4 py-2.5 flex items-center justify-between gap-3 shadow-inner z-20">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-1 rounded-lg transition-all shadow-xs cursor-pointer"
            >
              Approve & Resume
            </button>
            <button
              onClick={() => handleHITLApproval(false)}
              className="bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold px-3 py-1 rounded-lg transition-all shadow-xs cursor-pointer"
            >
              Reject Action
            </button>
          </div>
